...
public class Minesweeper {
	...
	public static void main(String[] args) {
		...
		String[][] mines = new String[row][col]; // see where is bombs and make
													// a matrix
		int[][] Findmines = new int[row][col]; // calculate the number of bombs
												// and make a matrix
		...
		for (int i = 0; i < row; i++) { // calculate the number bomb near by
			for (int j = 0; j < col; j++) { // 8 cases to check where is bomb
											// near by
				// [i-1][j-1]
				if (i > 0 && j > 0) {
					if (mines[i - 1][j - 1] == "*") {
		...
		for (int i = 0; i < row; i++) { // show bomb map
			for (int j = 0; j < col; j++) {
				if (mines[i][j] == "*") { // show where are bombs
					System.out.print("*");
				} else if (mines[i][j] == "-" && Findmines[i][j] == 0) { // show
																			// "-"
																			// there
																			// are
																			// no
